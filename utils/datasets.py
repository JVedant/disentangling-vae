...
class CelebAHQ(DisentangledDataset):
    """CelebAHQ Dataset from [1].

    CelebFaces Attributes Dataset High Quality (CelebAHQ) is a large-scale face attributes dataset
    with more than 200K celebrity images, each with 40 attribute annotations.
    The images in this dataset cover large pose variations and background clutter.
    CelebA has large diversities, large quantities, and rich annotations, including
    10,177 number of identities, and 202,599 number of face images.
    ...
    def __init__(self, root=os.path.join(DIR, '../data/celebahq/'), **kwargs):
        super().__init__(root, [transforms.ToTensor()], **kwargs)
